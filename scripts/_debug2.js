...
function matchDropOffLocationWithSpan(text) {
  ...
  for (const [canonical, aliases] of Object.entries(DROP_OFF_LOCATIONS)) {
    ...
    for (const alias of aliases) {
      ...
      if (idx !== -1) {
        // Find where the alias ends in the original text, then skip any
        // trailing address/junk until an item pattern starts (digit).
        let end = idx + strippedAlias.length
        // Skip non-digit chars that are part of the location address
        // e.g. "love fridge 21st and Cali" → skip "21st and Cali"
        const remainder = text.slice(end)
        const itemStart = remainder.match(/\s+(\d)/)
        if (itemStart) {
          // Check if the digit is preceded by location-like text (address)
          // vs being a quantity. Try parsing from digit onward.
          const fromDigit = remainder.slice(itemStart.index).trim()
          const testItems = parseItemsFromMessage(fromDigit)
          if (testItems.length > 0) {
            end = end + itemStart.index
          }
        }
        return { name: canonical, endIndex: end }
      }
    }
  }
  return null
...
function classifyMessage(text) {
  ...
  // Also handles mid-message and "scooped this from" with filler words
...
function splitByDropOff(sectionText, rescueLocation, defaultDropOff) {
  ...
  const dropPattern = /(?:dropped(?:\s+off)?|delivered|took)\s+(?:at\s+|to\s+)?(.+)/i
  ...
  if (dropLineIdx === -1 && lines.length <= 2) {
    ...
    const midLineMatch = fullText.match(/(.+?)\s+(?:dropped(?:\s+off)?|delivered|took)\s+(?:at\s+|to\s+)?(.+)/i)
    if (midLineMatch) {
      ...
      if (span) {
        ...
        const afterItems = parseItemsFromMessage(afterItemsText || afterDrop)
